...
module.exports = function (log, error) {
  ...
  Memory.prototype.createUnblockCode = function (uid, code, maxAttempts) {
    uid = uid.toString('hex')
    var row = unblockCodes[uid] || (unblockCodes[uid] = {})
    row[code] = {timeStamp: Date.now(), attemptsLeft: maxAttempts }
  ...
  Memory.prototype.tryUnblockCode = function (uid, code) {
    var row = unblockCodes[uid.toString('hex')]
    if (! row || ! row[code]) {
      return P.reject(error.notFound())
    }
    var timestamp = row[code].timestamp
    var attemptsLeft = row[code].attemptsLeft
    if (attemptsLeft === 1) {
      delete row[code]
    } else {
      row[code].attemptsLeft--
    }
    return P.resolve({ createdAt: timestamp })
  }
