...
module.exports = function (log, error) {
  ...
  Memory.prototype.accountRecord = function (emailBuffer) {
    ...
    // If email record is not on the emails table, fall back to check the record
    // on the account table
    var account
    if (! emails[normalizedEmail]) {
      return this.emailRecord(emailBuffer)
        .then((accountResult) => {
  ...
  Memory.prototype.accountEmails = function (uid) {
    ...
    return getAccountByUid(uid)
      .then(function (account) {
        ...
        userEmails[0].isPrimary = ! hasOtherPrimary

        return P.resolve(userEmails)
      })
  }

  Memory.prototype.setPrimaryEmail = function (uid, email, verifyHash) {
    let account

    // Check if email exists on email table
    // For time being, you can only `swap` a secondary and primary email address
    if (! emails[email]) {
      return P.reject(error.unknown())
    }

    return getAccountByUid(uid)
      .then((data) => {
        // Check if user's account email is in emails table
        // and add to emails table if needed.
        account = data
        const accountEmail = account.normalizedEmail
        if (! emails[accountEmail]) {
          emails[accountEmail] = {
            email: account.email,
            normalizedEmail: account.normalizedEmail,
      ...
      .then(() => {
        // If a verifyHash was passed in, then they signals that we should change
        // email on account table to secondary email.
        const emailData = emails[email]
        if (verifyHash) {
          account.email = emailData.email
          account.normalizeEmail = account.email.toLowerCase()
          account.emailCode = emailData.emailCode
          account.emailVerified = emailData.isVerified
          account.verifyHash = verifyHash
          delete emails[email]
        } else {
          emailData.isPrimary = true
        }
      })
  }
