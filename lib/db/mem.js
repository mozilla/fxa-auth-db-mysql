    ...
    return this.accountDevices(uid).then(function (devices) {
      ...
      var sessions = Object.keys(sessionTokens).filter(function (key) {
        ...
        var deviceInfo = devices.find(function (device) {
          return device.sessionTokenId === key
        })
        ...
        if (deviceInfo) {
          // device information
          session.deviceId = deviceInfo.id
          session.deviceName = deviceInfo.name
          session.deviceType = deviceInfo.type
          session.deviceCreatedAt = deviceInfo.createdAt
          session.deviceCallbackURL = deviceInfo.callbackURL
          session.deviceCallbackPublicKey = deviceInfo.callbackPublicKey
          session.deviceCallbackAuthKey = deviceInfo.callbackAuthKey
        }
