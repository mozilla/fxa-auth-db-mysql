...
module.exports = function (log, error) {
  ...
  Memory.prototype.clientInstance = function(uid, id) {
    id = id.toString('hex')
    return getAccountByUid(uid).then(account => {
      const instance = account.clientsInstances[id]
      if (! instance) {
        throw error.notFound()
      }
      return instance
  ...
  Memory.prototype.clientsInstances = function(uid) {
    return getAccountByUid(uid).then(account => {
      // No Object.values :(
      return Object.keys(account.clientsInstances).map(id => {
        return account.clientsInstances[id]
      })
    })
  }

  Memory.prototype.deleteClientInstance = function(uid, id) {
    id = id.toString('hex')
    return getAccountByUid(uid).then(account => {
      if (! account.clientsInstances[id]) {
        throw error.notFound()
      }
      delete account.clientsInstances[id]
      return {}
    })
  }

  Memory.prototype.upsertClientInstance = function(uid, id, info) {
    const idHex = id.toString('hex')
    return getAccountByUid(uid).then(account => {
      const {clientId, name, pushEndpoint, pushPublicKey, pushAuthKey, availableCommands} = info
      if (! account.clientsInstances[idHex]) {
        account.clientsInstances[idHex] = {
          uid,
          id,
          clientId,
        }
      }
      Object.assign(account.clientsInstances[idHex], {
        name,
        pushEndpoint,
        pushPublicKey,
        pushAuthKey,
