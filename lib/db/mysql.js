...
module.exports = function (log, error) {
  ...
  function makeStatementsToAddAvailableCommands(uid, clientInstanceId, info) {
    const availableCommands = info.availableCommands || {}
    return Object.keys(availableCommands).reduce((acc, commandName) => {
      ...
      acc.push({
        ...
        params: [uid, clientInstanceId, commandName, commandData]
      })
      return acc
    }, [])
  }
  
  // XXX TODO: documentation here.
  const CLIENTS_INSTANCES = 'CALL clientInstances_1(?)'

  MySql.prototype.clientInstances = function (uid) {
    return this.readAllResults(CLIENT_INSTANCES, [uid])
      .then(rows => dbUtil.aggregateNameValuePairs(rows, 'clientInstanceId', 'commandName', 'commandData', 'availableCommands'))
  }


  // XXX TODO: documentation here.
  const CLIENT_INSTANCE = 'CALL clientInstance_1(?, ?)'

  MySql.prototype.clientInstance = function (uid, clientInstanceId) {
    return this.readAllResults(CLIENT_INSTANCE, [uid, clientInstanceId])
      .then(rows => dbUtil.aggregateNameValuePairs(rows, 'clientInstanceId', 'commandName', 'commandData', 'availableCommands'))
      .then(instances => {
  ...
  // XXX TODO: documentation here.
  const CLIENT_INSTANCE_FROM_TOKEN_VERIFICATION_ID = 'CALL clientInstanceFromTokenVerificationId_1(?, ?)'

  MySql.prototype.clientInstanceFromTokenVerificationId = function (uid, tokenVerificationId) {
    return this.readAllResults(CLIENT_INSTANCE_FROM_TOKEN_VERIFICAITON_ID, [uid, tokenVerificationId])
      .then(rows => dbUtil.aggregateNameValuePairs(rows, 'clientInstanceId', 'commandName', 'commandData', 'availableCommands'))
      .then(instances => {
        if (instances.length === 0) {
          throw error.notFound()
  ...
  // XXX TODO: documentation here.
  const CREATE_CLIENT_INSTANCE = 'CALL createClientInstance_1(?, ?, ?, ?, ?, ?, ?)'

  MySql.prototype.createClientInstance = function (uid, clientInstanceId, info) {
    const statements = [{
      sql: UPSERT_CLIENT_INSTANCE,
      params: [
        uid,
        clientInstanceId,
        info.clientId,
        ...
        info.pushPublicKey,
        info.pushAuthKey,
        info.pushEndpointExpired,
        info.uaBrowser,
        info.uaBrowserVersion,
        info.uaOS,
        info.uaOSVersion,
        info.uaDeviceType,
        info.uaFormFactor
      ...
      resultHandler(result) {
        // If the UPSERT_CLIENT_INSTANCE fails, no need to continue!
        if (result.affectedRows === 0) {
          log.error('MySql.upsertClientInstance', { err: result })
          throw error.notFound() // XXX TODO: what error here?
        }
      }
    }]

    if (info.hasOwnProperty('availableCommands')) {
      statements.push({ sql: PURGE_AVAILABLE_CLIENT_COMMANDS, params: [uid, clientInstanceId] },
        ...makeStatementsToAddAvailableCommands(uid, clientInstanceId, info))
    }
    return this.writeMultiple(statements)
  }

  // XXX TODO: documentation here.
  const UPDATE_CLIENT_INSTANCE = 'CALL updateClientInstance_1(?, ?, ?, ?, ?, ?, ?)'

  MySql.prototype.updateClientInstance = function (uid, clientInstanceId, info) {
    const statements = [{
      sql: UPSERT_CLIENT_INSTANCE,
      params: [
        uid,
  ...
  // XXX TODO: documentation here.
  const DELETE_CLIENT_INSTANCE = 'CALL deleteClientInstance_1(?, ?)'

  MySql.prototype.deleteClientInstance = function (uid, clientInstanceId) {
    return this.write(DELETE_CLIENT_INSTANCE, [ uid, clientInstanceId ], result => {
      if (result.affectedRows === 0) {
        log.error('MySql.deleteClientInstance', { err: result })
        throw error.notFound()
      }
      return result[0]
    })
  ...
  // TODO: docs here
  var SESSION_TOKEN = 'CALL sessionTokenWithClientInstance_1(?)'

  MySql.prototype.sessionToken = function (id) {
    return this.readAllResults(SESSION_TOKEN, [id])
      .then(rows => dbUtil.aggregateNameValuePairs(rows, 'clientInstanceId', 'clientInstanceCommandName', 'clientInstanceCommandData', 'clientInstanceAvailableCommands'))
      .then(results => {
        if (results.length === 0) {
          throw error.notFound()
        }
  ...
  // TODO: docs here
  var KEY_FETCH_TOKEN = 'CALL keyFetchTokenWithVerificationStatus_2(?)'

  MySql.prototype.keyFetchToken = function (tokenId) {
    return this.readFirstResult(KEY_FETCH_TOKEN, [tokenId])
  }
