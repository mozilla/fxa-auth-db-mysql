...
module.exports = function (log, error) {
  ...
  var CREATE_UNBLOCK_CODE = 'CALL createUnblockCode_2(?, ?, ?, ?)'

  MySql.prototype.createUnblockCode = function (uid, code, maxAttempts) {
    // hash the code since it's like a password
    code = createHash(uid, code)
    return this.write(
      CREATE_UNBLOCK_CODE,
      [ uid, code, Date.now(), maxAttempts],
      function (result) {
        return {}
      }
    )
  ...
  var TRY_UNBLOCK_CODE = 'CALL tryUnblockCode_1(?, ?)'

  MySql.prototype.tryUnblockCode = function (uid, code) {
    // hash the code since it's like a password
    code = createHash(uid, code)
    return this.write(
      TRY_UNBLOCK_CODE,
      [ uid, code ],
      function (result) {
        log.error('happened', {result: result})
        if (result.length === 0 || result[0].length === 0 || ! result[0][0].createdAt) {
          log.error('MySql.tryUnblockCode', { err: result })
          throw error.notFound()
        }
        return result[0][0]
      }
