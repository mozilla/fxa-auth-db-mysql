...
module.exports = function (log, error) {
  ...
  const PURGE_AVAILABLE_CLIENT_COMMANDS = 'CALL purgeAvailableClientCommands_1(?, ?)'
  const UPSERT_AVAILABLE_CLIENT_COMMAND = 'CALL upsertAvailableClientCommand_1(?, ?, ?, ?)'
  const PURGE_AVAILABLE_DEVICE_COMMANDS = 'CALL purgeAvailableCommands_1(?, ?)'
  const UPSERT_AVAILABLE_DEVICE_COMMAND = 'CALL upsertAvailableCommand_1(?, ?, ?, ?)'

  function makeStatementsToAddAvailableCommands(type, uid, id, info) {
    const sql = type === 'client' ? UPSERT_AVAILABLE_CLIENT_COMMAND : UPSERT_AVAILABLE_DEVICE_COMMAND
    ...
    return Object.keys(availableCommands).reduce((acc, commandName) => {
      ...
      acc.push({
        sql,
        params: [uid, id, commandName, commandData]
      })
      return acc
    }, [])
  }
  ...
  MySql.prototype.clientInstance = function (uid, id) {
    return this.readAllResults(CLIENT_INSTANCE, [uid, id])
  ...
  const CLIENTS_INSTANCES = 'CALL clientsInstances_1(?)'

  MySql.prototype.clientsInstances = function (uid) {
    return this.readAllResults(CLIENTS_INSTANCES, [uid])
      .then(rows => dbUtil.aggregateNameValuePairs(rows, 'id', 'commandName', 'commandData', 'availableCommands'))
  }

  const DELETE_CLIENT_INSTANCE = 'CALL deleteClientInstance_1(?, ?)'

  MySql.prototype.deleteClientInstance = function (uid, id) {
    return this.write(DELETE_CLIENT_INSTANCE, [ uid, id ], result => {
  ...
  const UPSERT_CLIENT_INSTANCE = 'CALL upsertclientInstance_1(?, ?, ?, ?, ?, ?, ?)'

  MySql.prototype.upsertClientInstance = function (uid, id, info) {
    const statements = [{
    ...
    if (info.hasOwnProperty('availableCommands')) {
      statements.push({sql: PURGE_AVAILABLE_CLIENT_COMMANDS, params: [uid, id]},
                      ...makeStatementsToAddAvailableCommands('client', uid, id, info))
    }
  ...
  MySql.prototype.createDevice = function (uid, deviceId, deviceInfo) {
    ...
    if (deviceInfo.hasOwnProperty('availableCommands')) {
      statements.push(...makeStatementsToAddAvailableCommands('device', uid, deviceId, deviceInfo))
    }
    return this.writeMultiple(statements)
  }
  ...
  MySql.prototype.updateDevice = function (uid, deviceId, deviceInfo) {
    ...
    if (deviceInfo.hasOwnProperty('availableCommands')) {
      statements.push({sql: PURGE_AVAILABLE_DEVICE_COMMANDS, params: [uid, deviceId]},
                      ...makeStatementsToAddAvailableCommands('device', uid, deviceId, deviceInfo))
    }
    return this.writeMultiple(statements)
  }
