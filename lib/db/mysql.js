...
module.exports = function (log, error) {
  ...
  MySql.prototype.createVerificationReminder = function (request) {
    var reminderData = request.body
    if (! reminderData || ! reminderData.uid || ! reminderData.type) {
      throw new Error('"uid", "type" are required')
    }

    reminderData.id = crypto.createHash('sha256').update(reminderData.uid + reminderData.type).digest()
    reminderData.uid = new Buffer(reminderData.uid)
  ...
  MySql.prototype.fetchReminders = function (request) {
  ...
  MySql.prototype.deleteReminder = function (request) {
    var reminderData = request.body
