...
module.exports = function (log, error) {
  ...
  MySql.prototype.sessionToken = function (id) {
    ...
    return this.readFirstResult(SESSION_TOKEN, [id])
      .then((res) => {
        session = res
        return this.accountEmails(session.uid)
      })
      .then((emails) => {
        emails.some((email) => {
          if (email.isPrimary) {
            session.emailVerified = email.isVerified
  ...
  MySql.prototype.accountRecord = function (emailBuffer) {
    return this.readFirstResult(EMAIL_RECORD, [emailBuffer.toString('utf8')])
      .then((accountRecord) => {
        return this.accountEmails(accountRecord.uid)
          .then((emails) => {
            accountRecord.emails = emails
            accountRecord.email = emails[0].email
            accountRecord.normalizedEmail = emails[0].normalizedEmail
            accountRecord.emailCode = emails[0].emailCode
            accountRecord.emailVerified = emails[0].isVerified
            return accountRecord
          })
      })
      .catch((err) => {
        return this.getSecondaryEmail(emailBuffer)
          .then((emailRecord) => {
            return this.account(emailRecord.uid)
              .catch(() => {
                throw err
              })
          })
