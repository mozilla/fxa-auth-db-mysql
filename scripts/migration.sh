...
echo "SELECT @patchlevel := value FROM `dbMetadata` WHERE `name` = `schema-patch-level`;\n">> "$FWD_SCHEMA"
echo "IF @patchlevel != $PREV_LEVEL THEN\n">> "$FWD_SCHEMA"
echo "SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO = 1643, MESSAGE_TEXT = 'Could not migrate to next patch';\n">> "$FWD_SCHEMA"
echo "END IF;\n">> "$FWD_SCHEMA"
